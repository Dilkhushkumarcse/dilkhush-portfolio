document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Read More / Show Less for Blog Posts ---
    const readMoreBtns = document.querySelectorAll('.read-more-btn');
    readMoreBtns.forEach(btn => {
        btn.addEventListener('click', e => {
            e.preventDefault();
            // Correctly target the parent .blog-post
            const parentPost = btn.closest('.blog-post');
            if (parentPost) {
                const moreText = parentPost.querySelector('.more-text');
                if (moreText) {
    ...
    if (track) {
        ...
        if (slides.length > 0) {
            ...
            // REMOVED faulty auto-expand logic from buttons
        ...
        fullscreenBtns.forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                if (post) {
                    ...
                    const shortText = post.querySelector(".short-text").innerText;
                    const moreText = post.querySelector(".more-text") ? post.querySelector(".more-text").innerText : "";
                    
                    modalTitle.innerText = title;
                    modalText.innerText = shortText + " " + moreText;
